...
- `docs/` — Markdown sources (the only committed content).
  ...
  - `docs/agent-api/**` — Agent API guides, concepts, migration, and reference.
...
- All public API contract facts (fields, enums, required/default, error codes) come from the OpenAPI documents generated by the backend (`/openapi/*.json`). Guides reference fields for workflows and do not duplicate full schemas.
